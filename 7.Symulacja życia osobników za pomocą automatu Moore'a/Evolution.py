...
def main():
	...
	class create_bot(): #Wprowadzanie dna
		def __init__(self, x, y, dna=False):
			...
			if dna != False:
				...
				for i in range(len(dna)):
					if random.random() < mutation_rate:  #wskaznik mutacji
						if i < 2:
				...
				#4 skladniki w dna - 1.Przyciaganie do zielonego jedzenia,2. przyciaganie do czerwonego jedzenia, 3.zasieg widzenia zielonego jedzenia, 4. zasieg widzenia czerwonego jedzenia 
			print(self.dna)
		...
		#funkcja generujaca nowa populacje, zabija stare poprzez stworzenie klona samego siebie 
		def reproduce(self):
	...
	#na poczatku bedziemy miec 10 losowych botow
	for i in range(10):
		bots.append(create_bot(random.uniform(0,game_width),random.uniform(0,game_height)))
	running = True
	while(running):
		gameDisplay.fill(black)
		#jeżeli na ekranie znajdzie sie mniej niz 5 botow, dodamy kolejny
		if len(bots)<5 or random.random() < 0.0001:
			bots.append(create_bot(random.uniform(0,game_width),random.uniform(0,game_height)))
		#losowe dodanie jedzenia 
		...
		for bot in bots[::-1]:
			...
			bot.eat(poison, 1)
			bot.boundaries()
			#poszukiwania bota (pygame.mouse.get_pos())
			bot.update()
		...
		#Rysowanie okregu (gameDisplay, bot.colour, (int(self.position[0]), int(self.position[1])), 10)
		for i in poison:
