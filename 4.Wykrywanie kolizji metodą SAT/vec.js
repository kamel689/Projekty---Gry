...
Vector.prototype.unfloat = function () {
    this.x = Math.round(this.x);  //zwracamy liczbe całkowitą
    this.y = Math.round(this.y);
    return this;
};
Vector.prototype.toFixed = function (precision) {
    if (typeof precision === 'undefined') { precision = 8; }
    this.x = this.x.toFixed(precision); //przeformatowanie na notacje stałoprzecinkową
    this.y = this.y.toFixed(precision);
    return this;
};

Vector.prototype.rad2deg = function (rad) { //promien
    return rad * 180 / Math.PI;
};
Vector.prototype.deg2rad = function (deg) { // stopien
